...
procedure hierarchical_dm_schedulability_analysis is
...
begin
   ...
   Put_Line
     (" Result for gaps" &
      compute_partition_internal_gaps (deployment_test, 4, 200.0, False)'Img);
   ...
   Put_Line
     (" Result for servers interference" &
      compute_server_interference (sys.deployments, T1_ref, 200.0, False)'Img);
   Put_Line (" Result for T1 WCRT" & compute_wcrt (sys, T1_ref, False)'Img);
   Put_Line (" Result for T2 WCRT" & compute_wcrt (sys, T2_ref, False)'Img);
   Write_To_Xml_File (sys, "hierarchical_double_deployment.xml");

end hierarchical_dm_schedulability_analysis;
