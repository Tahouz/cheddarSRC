...
procedure scm  is
   ...
   begin
      Put_Line
        ("scm  : transform a Cheddar xmlv3 model of AADLInspector to be compliant with SCM. The SCM model is printed to the output.");
      New_Line;
      Put_Line
        ("Check Cheddar home page for details :  http://beru.univ-brest.fr/~singhoff/cheddar ");
      New_Line;
      New_Line;
      Put_Line ("Usage : scm  [switch] ");
...
begin
   ...
   a_network:=search_network(input_sys.networks, to_unbounded_string("spacewire1"));
   compute_spacewire_transformation
              (input_sys,
               spacewire_network_ptr (a_network),
               scm_sys);

   systems.write_to_xml_file (scm_sys, "after_SCM_" & input_file_name);
