...
procedure paes_method is

   procedure paes_F2T_architecture_exploration is new paes_general_form
     (init => chromosome_data_manipulation.init_F2T,
      mutate => chromosome_data_manipulation.mutate_F2T,
...
begin
	    ...
	    if Full_Switch =  "preprocessed_initial_sol" then
         ...
         when 'D' =>
            if Full_Switch = "DW_capacity_encrypt" then
               ...
               Put_Debug ("Capacity of encrypter for autopilot = " & DW_capacity_encrypt2'img);
            elsif Full_Switch = "DW_capacity_decrypt3" then
               DW_capacity_decrypt3 := Integer'Value(GNAT.Command_Line.Parameter);
               Put_Debug ("Capacity of decrypter for autopilot= " & DW_capacity_decrypt2'img);
            end if;
         when 'U' =>
  ...
  -- genes_init := genes;
   ...
   A_solution.chrom_partition_PU(1) := 1;
...
--Hyperperiod_of_Initial_Taskset := 2250;
 --  Hyperperiod_of_Initial_Taskset := 4250;
   Hyperperiod_of_Initial_Taskset := 6125;
...
--     end if;
   ...
   for i in 1..arclength loop
      ...
      for l in 1..nb_partitions loop
         Append (Data, arc(i).chrom_partition_PU(l)'Img & " ");
      end loop;
      Append (Data,  ASCII.LF);
