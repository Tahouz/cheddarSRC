...
begin
    ...
    -- Read the base system, assume that this part is provided by AADL/Osate
    --
    Systems.Read_From_Xml_File (Sys,
                                Project_File_Dir_List,
                                "input.xml");
    
    -- Option 1 : All cfgs are stored in one file
    --        
    --      Systems.Read_From_Xml_File (Sys_CFG,
    --                                  Project_File_Dir_List,
    --                                  "cfgs.xml");                
    --      Sys.CFGs := Sys_CFG.CFGs;
    
    -- Option 2: CFGs are stored in separate files
    --
    ...
    loop
        ...
        if (A_Task.cfg_name /= empty_string) then
            Systems.Read_From_Xml_File (Sys_CFG, Project_File_Dir_List, A_Task.cfg_name);
