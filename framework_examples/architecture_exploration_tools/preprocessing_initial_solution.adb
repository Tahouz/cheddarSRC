...
begin
   ...
   Number_of_resources_functional_spec := Get_Number_Of_Resource_From_Processor
     (Functional_spec_system.Resources, To_Unbounded_String ("processor1"));
   ...
   Number_of_functions_functional_spec := Get_Number_Of_Task_From_Processor
     (Functional_spec_system.Tasks, To_Unbounded_String ("processor1"));
   ...
   for i in 1 .. genes loop
     One_to_one_mapping_solution.chrom(i) := i;
   end loop;
   ...
   for i in 1 .. Integer(Number_of_functions_functional_spec) loop
      ...
      A_preprocessed_initial_solution.chrom(i) := 0;
   end loop;
...
--        end loop;
	 ...
	 if (A_preprocessed_initial_solution.chrom (Funct (j)) = 0) then
	    ...
	    for k in 1 .. Nb_functions_accessing_the_resource loop
		  ...
		  if tmp_mod = 0 and A_preprocessed_initial_solution.chrom (Funct (k)) = 0 then
			...
			Is_assigned := true;
			if A_preprocessed_initial_solution.chrom (Funct (j)) = 0 then
			   A_preprocessed_initial_solution.chrom (Funct (j)) := A_task_index;
			end if;
			A_preprocessed_initial_solution.chrom (Funct (k)) := A_task_index;
		     end if;
   ...
   for j in 1 .. genes loop
      if A_preprocessed_initial_solution.chrom (j) = 0 then
	 A_preprocessed_initial_solution.chrom (j) := A_task_index;
	 A_task_index := A_task_index + 1;
      end if;
   end loop;
   print_genome (A_preprocessed_initial_solution);
   ...
   for l in 1..genes loop
      Append (Data, A_preprocessed_initial_solution.chrom (l)'Img & " ");
   end loop;
   ...
   Create_system (preprocessed_initial_sol_system);
