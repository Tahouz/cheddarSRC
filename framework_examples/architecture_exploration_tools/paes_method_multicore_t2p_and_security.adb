...
procedure paes_method_multicore_t2p_and_security is

   package paes_T2P_architecture_exploration is new Paes.general_form
     (init =>
        Paes.chromosome_Data_Manipulation_multicore_t2p_and_security
          .init_multicore_T2P,
      mutate =>
        Paes.chromosome_Data_Manipulation_multicore_t2p_and_security
          .mutate_multicore_T2P,
      evaluate =>
        Paes.objective_functions.multicore_t2p_and_security
...
begin
   ...
   loop
      ...
      case GNAT.Command_Line.Getopt
        ("h n: nb_cores: nb_partitions: nb_app: sched: iter: i: u: p: fitness: select: n_diff_periods: t2c_exploration_version: t2p_exploration_version: intra_partition_safe:  n_res: rsf: partition_period: csr: preprocessed_initial_sol")
      is
         when ASCII.NUL =>
            exit;
         ...
         when 'n' =>
            ...
            if Full_Switch = "nb_cores" then
               nb_cores := Integer'value (GNAT.Command_Line.Parameter);
               put_debug ("Number of cores = " & nb_cores'img);
            end if;
         ...
         when 't' =>
            if Full_Switch = "t2p_exploration_version" then
               t2p_exploration_version :=
                 Integer'value (GNAT.Command_Line.Parameter);
               if (t2p_exploration_version = 1) then
                  put_debug
                    ("The exploration is " &
                     ": T2P App_grain algorithm based");
               elsif (t2p_exploration_version = 2) then
                  put_debug
                    ("The exploration is " &
                     ": T2P Mix_grain algorithm based");
               elsif (t2p_exploration_version = 3) then
                  put_debug
                    ("The exploration is " &
                     ": T2P Task_grain algorithm based");
               end if;
            end if;
            if Full_Switch = "t2c_exploration_version" then
               t2c_exploration_version :=
                 Integer'value (GNAT.Command_Line.Parameter);
               if (t2c_exploration_version = 1) then
                  put_debug
                    ("The exploration is " &
                     ": T2C App_grain algorithm based");
               elsif (t2c_exploration_version = 2) then
                  put_debug
                    ("The exploration is " &
                     ": T2C Mix_grain algorithm based");
               elsif (t2c_exploration_version = 3) then
                  put_debug
                    ("The exploration is " &
                     ": T2C Task_grain algorithm based");
               end if;
            end if;
...
--   archive               := 200; -- the archive size is fixed to 200
   archive := 1000; -- the archive size is fixed to 200

   c := new solution_t2p_t2c;
   m := new solution_t2p_t2c;
   for i in 1 .. archive loop
      arc (i) := new solution_t2p_t2c;
      tmp (i) := new solution_t2p_t2c;
   end loop;
...
--     nb_task_per_app(1):=4; --Rosace
--     nb_task_per_app(2):=4;  --CFAR
--     nb_task_per_app(3):=4; --Rosace
--     nb_task_per_app(4):=4;  --CFAR
--     nb_task_per_app(5):=4; --Rosace
--     nb_task_per_app(6):=4;  --CFAR

--     --initialize the number of tasks per application
   nb_task_per_app (1) := 15; --Rosace
   nb_task_per_app (2) := 4;  --CFAR
   nb_task_per_app (3) := 15; --Rosace
   nb_task_per_app (4) := 4;  --CFAR
   nb_task_per_app (5) := 15; --Rosace
   nb_task_per_app (6) := 4;  --CFAR
   --Experiments1: 2 app
--     nb_task_per_app(1):=15; --Rosace
--     nb_task_per_app(2):=7;  --JPEG
--     nb_task_per_app(3):=15; --Rosace
--     nb_task_per_app(4):=7;  --JPEG
--     nb_task_per_app(5):=15; --Rosace
--     nb_task_per_app(6):=7;  --JPEG
...
--              nb_task_per_app(6):=5;  --Autopilot3
   ...
   encrypter_values (3) := 1; -- App3
   encrypter_values (4) := 1; -- App4
   encrypter_values (5) := 1; -- App5
   encrypter_values (6) := 1; -- App6
   ---Initialize decrypter values for each app---
   decrypter_values (1) := 1;  --app1
   decrypter_values (2) := 1; -- App2
   decrypter_values (3) := 1; -- App3
   decrypter_values (4) := 1; -- App4
   decrypter_values (5) := 1; -- App5
   decrypter_values (6) := 1; -- App6
   ---Initialize decrypter values for each app---
   Hash_values (1) := 1;  --app1
   Hash_values (2) := 1; -- App2
   Hash_values (3) := 1; -- App3
   Hash_values (4) := 1; -- App4
   Hash_values (5) := 1; -- App5
   Hash_values (6) := 1; -- App6

--     ---Initialize encrypters values for each app---
--     encrypter_values(1):= 1;  --app1
--     encrypter_values(2):= 1683; -- App2
--     encrypter_values(3):= 1; -- App3
--     encrypter_values(4):= 1683; -- App4
--     encrypter_values(5):= 1; -- App5
--     encrypter_values(6):= 1683; -- App6
--     ---Initialize decrypter values for each app---
--     decrypter_values(1):= 1;  --app1
--     decrypter_values(2):= 1683; -- App2
--     decrypter_values(3):= 1; -- App3
--     decrypter_values(4):= 1683; -- App4
--     decrypter_values(5):= 1; -- App5
--     decrypter_values(6):= 1683; -- App6
--     ---Initialize decrypter values for each app---
--     hash_values(1):= 1;  --app1
--     hash_values(2):= 1017; -- App2
--     hash_values(3):= 1; -- App3
--     hash_values(4):= 1017; -- App4
--     hash_values(5):= 1; -- App5
--     hash_values(6):= 1017; -- App6

   --Initialize communication mechanisms overhead for each app--

   ---intra partition communications per application --
   display_blackboard_time_values (1) := 1; --app1
   display_blackboard_time_values (2) := 1; --app2
   display_blackboard_time_values (3) := 1; --app2
   display_blackboard_time_values (4) := 1; --app2
   display_blackboard_time_values (5) := 1; --app2
   display_blackboard_time_values (6) := 1; --app2

   read_blackboard_time_values (1) := 1;    --app1
   read_blackboard_time_values (2) := 1;    --app2
   read_blackboard_time_values (3) := 1;    --app1
   read_blackboard_time_values (4) := 1;    --app1
   read_blackboard_time_values (5) := 1;    --app1
   read_blackboard_time_values (6) := 1;    --app1

   ---inter partition communications per application --

   write_sampling_port_time_values (1) := 1; --app1
   write_sampling_port_time_values (2) := 1; --app2
   write_sampling_port_time_values (3) := 1; --app3
   write_sampling_port_time_values (4) := 1; --app4
   write_sampling_port_time_values (5) := 1; --app5
   write_sampling_port_time_values (6) := 1; --app6

   read_sampling_port_time_values (1) := 1; --app1
   read_sampling_port_time_values (2) := 1; --app2
   read_sampling_port_time_values (3) := 1; --app3
   read_sampling_port_time_values (4) := 1; --app4
   read_sampling_port_time_values (5) := 1; --app5
   read_sampling_port_time_values (6) := 1; --app6

--     write_sampling_port_time_values(1):= 28; --app1
--     write_sampling_port_time_values(2):= 28; --app2
--     write_sampling_port_time_values(3):= 28; --app3
--     write_sampling_port_time_values(4):= 28; --app4
--     write_sampling_port_time_values(5):= 28; --app5
--     write_sampling_port_time_values(6):= 28; --app6
--
--     read_sampling_port_time_values(1):= 28; --app1
--     read_sampling_port_time_values(2):= 28; --app2
--     read_sampling_port_time_values(3):= 28; --app3
--     read_sampling_port_time_values(4):= 28; --app4
--     read_sampling_port_time_values(5):= 28; --app5
--     read_sampling_port_time_values(6):= 28; --app6

   ---intercore communications per application --Experiments1--> overheads=0
   write_intercore_overhead_values (1) := 0; --app1
   write_intercore_overhead_values (2) := 0; --app2
   write_intercore_overhead_values (3) := 0; --app3
   write_intercore_overhead_values (4) := 0; --app4
   write_intercore_overhead_values (5) := 0; --app5
   write_intercore_overhead_values (6) := 0; --app6

   read_intercore_overhead_values (1) := 0; --app1
   read_intercore_overhead_values (2) := 0; --app2
   read_intercore_overhead_values (3) := 0; --app3
   read_intercore_overhead_values (4) := 0; --app4
   read_intercore_overhead_values (5) := 0; --app5
   read_intercore_overhead_values (6) := 0; --app6

   ---intercore communications per application --Experiments1--> overheads=inter-partition overhead

--     write_intercore_overhead_values(1):= 28; --app1
--     write_intercore_overhead_values(2):= 28; --app2
--     write_intercore_overhead_values(3):= 28; --app1
--     write_intercore_overhead_values(4):= 28; --app1
--     write_intercore_overhead_values(5):= 28; --app1
--     write_intercore_overhead_values(6):= 28; --app1
--
--     read_intercore_overhead_values(1):= 28; --app1
--     read_intercore_overhead_values(2):= 28; --app2
--     read_intercore_overhead_values(3):= 28; --app1
--     read_intercore_overhead_values(4):= 28; --app1
--     read_intercore_overhead_values(5):= 28; --app1
--     read_intercore_overhead_values(6):= 28; --app1

   ---intercore communications per application --Experiments1--> overheads=4 * inter-partition overhead

--      write_intercore_overhead_values(1):= 112; --app1
--     write_intercore_overhead_values(2):= 112; --app2
--     write_intercore_overhead_values(3):= 112; --app1
--     write_intercore_overhead_values(4):= 112; --app1
--     write_intercore_overhead_values(5):= 112; --app1
--     write_intercore_overhead_values(6):= 112; --app1
--
--     read_intercore_overhead_values(1):= 112; --app1
--     read_intercore_overhead_values(2):= 112; --app2
--     read_intercore_overhead_values(3):= 112; --app1
--     read_intercore_overhead_values(4):= 112; --app1
--     read_intercore_overhead_values(5):= 112; --app1
--     read_intercore_overhead_values(6):= 112; --app1

   num_app   := 1;
   app_limit := nb_task_per_app (1);

   for i in 1 .. 3 * genes loop
      -- Put all the tasks in the same partition; each instance of a task in a different partition
      -- 3*genes car safety --> triplication
      if (i <= genes) then
         A_solution.chrom_t2p (i) := 1;
      elsif (genes < i) and (i <= 2 * genes) then
         A_solution.chrom_t2p (i) := 2;
      elsif (2 * genes < i) and (i <= 3 * genes) then
         A_solution.chrom_t2p (i) := 3;
      end if;

      -- Put all the tasks in the same core
      A_solution.chrom_t2c (i) := 1;
      -- define task to application assignment
      -- for task_to_app(i); index for tasks and value for application
      if (num_app <= 3 * nb_app) then
...
--   Hyperperiod_of_Initial_Taskset := 2250;
   ...
   -- we use the CheddarADL design described in the xml file given in argument
      ...
      Put_Line ("start safety");
      Safety (Initial_System, A_solution);
      Put_Line ("end safety");
   ----------------------------
   ----------------------------
   ...
   A_Duration :=
     (Ends - Start) /
     86400; --/86400 pour ne pas dépasser la limite de Day_duration a-calend.adb
   Put ("Paes_runtime / 86400" & ASCII.HT & ":");
   Put (A_Duration, 8, 8, 0);
   Put (" * 86400 seconds");
   put_debug ("");
   ...
   for i in 1 .. arclength loop
      ...
      Append
        (Data,
         ...
         ":  ");
      Put ("Solution " & i'img & ":  ");
      print_genome (solution_t2p_t2c (arc (i).all));

      for l in 1 .. genes loop
         Append (Data, solution_t2p_t2c (arc (i).all).chrom_t2p (l)'img & " ");
      end loop;
      Append (Data, ASCII.LF);

      for l in 1 .. genes loop
         Append (Data, solution_t2p_t2c (arc (i).all).chrom_t2c (l)'img & " ");
      end loop;
      Append (Data, ASCII.LF);
      ...
      for l in 1 .. genes_com loop
         Append
           (Data,
            solution_t2p_t2c (arc (i).all).chrom_com (l).mode'img &
            " " &
            To_String
              (solution_t2p_t2c (arc (i).all).chrom_com (l).source_sink) &
            "    ");
      end loop;
      Append (Data, ASCII.LF);
      ...
      Create_system (Sys, solution_t2p_t2c (arc (i).all));
      Transform_Chromosome_To_CheddarADL_Model
        (Sys,
         solution_t2p_t2c (arc (i).all));
      write_to_xml_file
        (a_system  => Sys,
         file_name =>
           To_String
      ...
      Append
        (Data,
         "Number of emission for inter core communications  : " &
         com_cost (6)'img &
         ASCII.LF);
      Append
        (Data,
         "Number of reception for inter core communications  : " &
         com_cost (7)'img &
         ASCII.LF);
...
end paes_method_multicore_t2p_and_security;
