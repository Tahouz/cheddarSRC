...
package body applicability_constraint.unsimultaneous_release_time_constraint is
   ...
   function apply_query3_condition (t4 : generic_task_ptr) return Boolean is
   begin

      return (t4.task_type = periodic_type);
   ...
   function apply_query2_condition (t2 : generic_task_ptr) return Boolean is
      t3 : generic_task_ptr;
   begin
      get_element_number (context.tasks, t3, tasks_range (1));
      return not (t2.start_time = t3.start_time);
...
end applicability_constraint.unsimultaneous_release_time_constraint;
