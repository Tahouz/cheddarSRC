...
package body applicability_constraint.arbitrary_deadlines is

   function apply (sys : system) return Boolean is
   begin
      context := sys;

      return not applicability_constraint
          .period_smaller_than_deadline_constraint
          .apply
          (sys) and
        not applicability_constraint.period_equal_deadline_constraint.apply
          (sys) and
        not applicability_constraint.period_larger_than_deadline_constraint
          .apply
          (sys);

   end apply;

end applicability_constraint.arbitrary_deadlines;
