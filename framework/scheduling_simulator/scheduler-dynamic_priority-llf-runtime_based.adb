...
package body scheduler.dynamic_priority.llf.runtime_based is

   procedure initialize (a_scheduler : in out llf_runtime_based_scheduler) is
   begin
      reset (a_scheduler);
   ...
   function copy
     (a_scheduler : in llf_runtime_based_scheduler)
      return generic_scheduler_ptr
   is
      ptr : llf_runtime_based_scheduler_ptr;

   begin

      ptr := new llf_runtime_based_scheduler;
   ...
   procedure do_election
     (my_scheduler       : in out llf_runtime_based_scheduler;
         ...
         if not si.tcbs (i).already_run_at_current_time then
            if (si.tcbs (i).tsk.cpu_name = processor_name) then
               if check_core_assignment (my_scheduler, si.tcbs (i)) then
                  if (si.tcbs (i).wake_up_time <= current_time) then

                     time_passed := current_time - si.tcbs (i).wake_up_time;
                     deadline    :=
                       dynamic_priority_tcb_ptr (si.tcbs (i)).tsk.deadline;
                     rest_of_capacity := si.tcbs (i).rest_of_capacity;

                     laxity := deadline - (rest_of_capacity + time_passed);

                     if (laxity < smallest_laxity) and
...
end scheduler.dynamic_priority.llf.runtime_based;
