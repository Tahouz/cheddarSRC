...
-- Last update :
--    $Rev: 5257 $
--    $Date: 2024-11-23 23:08:10 +0100 (sam., 23 nov. 2024) $
--    $Author: singhoff $
------------------------------------------------------------------------------
------------------------------------------------------------------------------
...
package body scheduler.dynamic_priority.edf is

   procedure initialize (a_scheduler : in out edf_scheduler) is
   begin
      reset (a_scheduler);
   ...
   function copy
     (a_scheduler : in edf_scheduler) return generic_scheduler_ptr
   is
      ptr : edf_scheduler_ptr;

   begin

      ptr := new edf_scheduler;
   ...
   begin
      put_debug ("Call Do_Election: EDF");
      loop
