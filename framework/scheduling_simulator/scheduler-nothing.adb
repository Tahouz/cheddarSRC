...
package body scheduler.nothing is

   procedure initialize (a_scheduler : in out no_scheduler) is
   begin
      reset (a_scheduler);
      a_scheduler.parameters.scheduler_type := no_scheduling_protocol;
   end initialize;

   function copy
     (a_scheduler : in no_scheduler) return generic_scheduler_ptr
   is
   begin
   ...
   procedure put (my_scheduler : in no_scheduler) is
   begin
