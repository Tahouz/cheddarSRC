...
package body scheduler_io is
-- gets input and stuffs it into 'buf'.  number of characters read, or YY_NULL,
-- is returned in 'result'.

   string_ptr : Positive;
   to_parse   : Unbounded_String;
   ...
   begin
      if to_parse = empty_string then
         result := 0;
         return;
      end if;

      while (i <= max_size) loop
         if string_ptr = Length (to_parse) + 1 then
            yy_eof_has_been_seen := True;
            result               := i - 1;
            return;

         else
            buf (loc)  := Element (to_parse, string_ptr);
...
--     EOB_ACT_END_OF_FILE - end of file
   ...
   begin
      ...
      if (yy_eof_has_been_seen) then
         ...
         -- read in more data
         YY_INPUT
           (yy_ch_buf (number_to_move .. yy_ch_buf'last),
            yy_n_chars,
            num_to_read);
      end if;
...
-- default yywrap function - always treat EOF as an EOF
   ...
   procedure Open_Input (fname : in Unbounded_String) is
   begin
      yy_init    := True;
      to_parse   := fname;
      string_ptr := 1;
