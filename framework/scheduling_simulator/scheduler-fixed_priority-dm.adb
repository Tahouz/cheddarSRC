...
package body scheduler.fixed_priority.dm is

   procedure initialize (a_scheduler : in out dm_scheduler) is
   begin
      reset (a_scheduler);
      a_scheduler.parameters.scheduler_type := deadline_monotonic_protocol;
   end initialize;

   function copy
     (a_scheduler : in dm_scheduler) return generic_scheduler_ptr
   is
      ptr : dm_scheduler_ptr;

   begin

      ptr := new dm_scheduler;
