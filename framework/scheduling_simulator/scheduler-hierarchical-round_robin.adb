...
package body scheduler.hierarchical.round_robin is

   procedure initialize
     (a_scheduler : in out hierarchical_round_robin_scheduler)
   is
   begin
      reset (a_scheduler);
   ...
   function copy
     (a_scheduler : in hierarchical_round_robin_scheduler)
      return generic_scheduler_ptr
   is
      ptr : hierarchical_round_robin_scheduler_ptr;

   begin

      ptr := new hierarchical_round_robin_scheduler;
   ...
   procedure check_before_scheduling
     (my_scheduler   : in hierarchical_round_robin_scheduler;
   ...
   procedure specific_scheduler_initialization
     (my_scheduler       : in out hierarchical_round_robin_scheduler;
   ...
   begin
      ...
      -- Reset the quantum of the global scheduler
      --
      my_scheduler.used_quantum := 0;
         ...
         if event_to_generate (address_space_activation) then
            ...
            a_item.activation_address_space :=
              address_space_scheduler_ptr
                ...
                .corresponding_address_space
                .name;
            a_item.duration := get_quantum (my_scheduler);
            add (result.all, 0, a_item);
         end if;
      end if;

-- Check if we must switch the activated/schedulable address space
-- With Round robin, We switch when the global scheduler quantum is exhausted
      --
      my_scheduler.used_quantum := my_scheduler.used_quantum + 1;
      if get_quantum (my_scheduler) >= my_scheduler.used_quantum then
...
end scheduler.hierarchical.round_robin;
