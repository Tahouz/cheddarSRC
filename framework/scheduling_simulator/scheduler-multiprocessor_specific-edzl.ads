...
package scheduler.multiprocessor_specific.edzl is

   type edzl_tcb is new tcb with record
      dynamic_deadline : Natural;
   end record;

   type edzl_tcb_ptr is access all edzl_tcb'class;

   procedure initialize (a_tcb : in out edzl_tcb);

   -------------------------------------------------

   type multiprocessor_edzl_scheduler is
     new multiprocessor_scheduler with private;
   type multiprocessor_edzl_scheduler_ptr is
     access all multiprocessor_edzl_scheduler'class;

   function build_tcb
     (my_scheduler : in multiprocessor_edzl_scheduler;
      a_task       :    generic_task_ptr) return tcb_ptr;

   procedure initialize (a_scheduler : in out multiprocessor_edzl_scheduler);
...
private
   ...
   type multiprocessor_edzl_scheduler is new multiprocessor_scheduler with
   null record;
