...
package body scheduler.time_sharing_based_on_cpu_usage is
   ...
   is
      ptr : time_sharing_based_on_cpu_usage_scheduler_ptr;

   begin

      ptr := new time_sharing_based_on_cpu_usage_scheduler;
   ...
   is
      ...
      less_executed_task : Natural     := Natural'last;
      i                  : tasks_range := 0;
   ...
   begin
            ...
            if (si.tcbs (i).tsk.cpu_name = processor_name) then
               if check_core_assignment (my_scheduler, si.tcbs (i)) then
                  if (si.tcbs (i).wake_up_time <= current_time) and
                    (si.tcbs (i).used_cpu < less_executed_task) and
                           ...
                           if options.with_resources then
                              ...
                              if is_ready then
                                 ...
                                 then
                                    ...
                                    less_executed_task := si.tcbs (i).used_cpu;
                                    elected            := i;
                                 end if;
                              end if;
                           end if;
      ...
      if less_executed_task = Natural'last then
         no_task := True;
      else
         no_task := False;
      end if;
