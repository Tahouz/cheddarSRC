...
-- Last update :
--    $Rev: 4254 $
--    $Date: 2022-11-16 14:01:04 +0100 (mer. 16 nov. 2022) $
--    $Author: singhoff $
------------------------------------------------------------------------------
------------------------------------------------------------------------------
...
package body scheduler.hierarchical.fixed_priority is

   procedure initialize (a_scheduler : in out hierarchical_fixed_priority_scheduler) is
   begin
      reset (a_scheduler);
      a_scheduler.parameters.scheduler_type := hierarchical_fixed_priority_protocol;
   end initialize;

   function copy
     (a_scheduler : in hierarchical_fixed_priority_scheduler)
      return generic_scheduler_ptr
   is
      ptr : hierarchical_fixed_priority_scheduler_ptr;

   begin

      ptr := new hierarchical_fixed_priority_scheduler;
   ...
   procedure check_before_scheduling
     (my_scheduler   : in hierarchical_fixed_priority_scheduler;
   ...
   procedure specific_scheduler_initialization
     (my_scheduler       : in out hierarchical_fixed_priority_scheduler;
...
end scheduler.hierarchical.fixed_priority;
