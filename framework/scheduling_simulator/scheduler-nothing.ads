...
package scheduler.nothing is

   type no_scheduler is new generic_scheduler with private;
   type no_scheduler_ptr is access all no_scheduler'class;

   procedure initialize (a_scheduler : in out no_scheduler);

   function copy (a_scheduler : in no_scheduler) return generic_scheduler_ptr;

   procedure put (my_scheduler : in no_scheduler);

   procedure check_before_scheduling
     (my_scheduler   : in no_scheduler;
      my_tasks       : in tasks_set;
      processor_name : in Unbounded_String);

   procedure do_election
     (my_scheduler       : in out no_scheduler;
...
private
   ...
   type no_scheduler is new generic_scheduler with null record;

end scheduler.nothing;
