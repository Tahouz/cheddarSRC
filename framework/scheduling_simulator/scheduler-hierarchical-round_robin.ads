...
package scheduler.hierarchical.round_robin is

   type hierarchical_round_robin_scheduler is
     new hierarchical_scheduler with private;
   type hierarchical_round_robin_scheduler_ptr is
...
private
   ...
   type hierarchical_round_robin_scheduler is new hierarchical_scheduler with
   record

      -- Current activated address space
      --
      address_space_index : scheduler_table_range := 0;

      -- Amount of time consumed by the current address space
      --
      used_quantum : Natural := 0;
   end record;
