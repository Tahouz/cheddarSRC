...
package body scheduler.fixed_priority.aperiodic_server.polling is

   procedure initialize
     (a_scheduler : in out polling_aperiodic_server_scheduler)
   is
   begin
      reset (a_scheduler);
   ...
   function copy
     (a_scheduler : in polling_aperiodic_server_scheduler)
      return generic_scheduler_ptr
   is
      ptr : polling_aperiodic_server_scheduler_ptr;

   begin

      ptr := new polling_aperiodic_server_scheduler;
   ...
   procedure do_election
     (my_scheduler       : in out polling_aperiodic_server_scheduler;
   ...
   begin
      ...
      -- Election : if Current_Time is within the server awake time limits
      -- we check if there are any aperiodic tasks to run
      -- if not the server falls back to sleep
      -- and only the periodic tasks are serviced until the next
      -- server's awakening time
      --
      no_task := True;
      if (current_time >= my_scheduler.current_server_wake_time) and
        (current_time <
         my_scheduler.current_server_wake_time +
           my_scheduler.parameters.capacity)
      then
         if (my_scheduler.aperiodic_highest_priority /= Natural'first) and
      ...
      -- If the server exhausted all its capacity
...
end scheduler.fixed_priority.aperiodic_server.polling;
