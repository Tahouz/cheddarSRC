   ...
   begin 
	...
	loop
		...
		if highiest_priority = Natural'first then
			...
			-- 2. Verify if the current task exceed its computing budget
			...
			-- update the sum of the quality achieved by the elected task
	...
	if current_time = si.simulation_length-1 then
		...
		si.lo_time_spend := (100 * si.lo_time_spend) / si.simulation_length;
      ...
      put_debug ("Call Do_Election: AMC : Elected : " & elected'img);
   end do_election;
