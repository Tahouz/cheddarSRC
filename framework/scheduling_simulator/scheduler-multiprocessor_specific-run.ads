...
package scheduler.multiprocessor_specific.run is

   type run_tcb is new tcb with record
      dual_server : run_server_dual_ptr;
   end record;

   type run_tcb_ptr is access all run_tcb'class;

   procedure initialize (a_tcb : in out run_tcb);

   -------------------------------------------------

   type multiprocessor_run_scheduler is
     new multiprocessor_scheduler with private;
   type multiprocessor_run_scheduler_ptr is
     access all multiprocessor_run_scheduler'class;

   function build_tcb
     (my_scheduler : in multiprocessor_run_scheduler;
      a_task       :    generic_task_ptr) return tcb_ptr;

   procedure initialize (a_scheduler : in out multiprocessor_run_scheduler);
...
private
   ...
   type global_run_scheduler is record
      n_cores         : Natural := 0;
      time_to_update  : Natural := 0;
      running_tasks   : run_servers_table;
      run_server_tree : run_server_primal_ptr;
   end record;
   type global_run_scheduler_ptr is access global_run_scheduler;

   type multiprocessor_run_scheduler is new multiprocessor_scheduler with
   record
      version : Natural := 0;
      core_id : Natural := 0;
   end record;
