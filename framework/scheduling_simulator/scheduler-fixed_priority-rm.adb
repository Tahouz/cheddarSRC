...
package body scheduler.fixed_priority.rm is

   procedure initialize (a_scheduler : in out rm_scheduler) is
   begin
      reset (a_scheduler);
      a_scheduler.parameters.scheduler_type := rate_monotonic_protocol;
   end initialize;

   function copy
     (a_scheduler : in rm_scheduler) return generic_scheduler_ptr
   is
      ptr : rm_scheduler_ptr;

   begin

      ptr := new rm_scheduler;
