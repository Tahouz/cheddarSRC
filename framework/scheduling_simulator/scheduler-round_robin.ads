...
package scheduler.round_robin is

   type round_robin_scheduler is new generic_scheduler with private;
   type round_robin_scheduler_ptr is access all round_robin_scheduler'class;

   type rr_tcb is new tcb with record
      task_quantum : Natural := 0;
   end record;

   type rr_tcb_ptr is access all rr_tcb'class;

   procedure initialize (a_tcb : in out rr_tcb);

   procedure initialize (a_scheduler : in out round_robin_scheduler);
   ...
   procedure put (my_scheduler : in round_robin_scheduler);
...
private
   ...
   type round_robin_scheduler is new generic_scheduler with null record;
