...
package body scheduler.multiprocessor_specific.llref is
   ...
   procedure initialize (a_tcb : in out llref_tcb) is
   begin
   ...
   procedure initialize
     (a_scheduler : in out multiprocessor_llref_scheduler)
   is
   begin
      reset (a_scheduler);
   ...
   is
      ptr : multiprocessor_llref_scheduler_ptr;

   begin

      ptr := new multiprocessor_llref_scheduler;
   ...
   begin
      ...
      loop
         ...
         if (si.tcbs (i).tsk.cpu_name = processor_name) then
            ...
            laxity :=
              Double
                (llref_tcb_ptr (si.tcbs (i)).tsk.deadline -
                 current_time -
                 si.tcbs (i).rest_of_capacity);

            llref_tcb_ptr (si.tcbs (i)).dynamic_deadline :=
...
--
      if options.with_minimize_preemption and previous_task_can_be_run then
         if llref_tcb_ptr (si.tcbs (my_scheduler.previously_elected))
             .dynamic_deadline =
           smallest_deadline
         then
