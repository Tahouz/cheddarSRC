...
package scheduler.hierarchical is

   type hierarchical_scheduler is abstract new generic_scheduler with private;
   type hierarchical_scheduler_ptr is access all hierarchical_scheduler'class;

   procedure specific_scheduler_initialization
     (my_scheduler       : in out hierarchical_scheduler;
...
private
   ...
   type hierarchical_scheduler is abstract new generic_scheduler with record

      -- The schedulers of each address space
      --
      local_scheduler : scheduler_table;

      -- Last release time of the current address space
      --
      last_release : Natural := 0;

   end record;

end scheduler.hierarchical;
