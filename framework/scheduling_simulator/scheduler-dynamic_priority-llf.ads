...
package scheduler.dynamic_priority.llf is

   type llf_scheduler is new dynamic_priority_scheduler with private;
   type llf_scheduler_ptr is access all llf_scheduler'class;

   procedure initialize (a_scheduler : in out llf_scheduler);

   function copy (a_scheduler : in llf_scheduler) return generic_scheduler_ptr;
...
private
   ...
   type llf_scheduler is new dynamic_priority_scheduler with null record;
