...
package body Paes.multicore_t2p_and_security is

   ------------------
   -- print_genome --
   ------------------

   procedure print_genome (s : solution_t2p_t2c) is

   begin

      Put_Line ("Tasks to partition assignement");
      for i in 1 .. genes loop
         Put (s.chrom_t2p (i)'img & " ");
      end loop;

      New_Line;

      Put_Line ("Tasks to cores assignement");
      for i in 1 .. genes loop
         Put (s.chrom_t2c (i)'img & " ");
      end loop;

      for i in 1 .. genes_com loop
         Put
           (s.chrom_com (i).mode'img &
            " " &
            To_String (s.chrom_com (i).source_sink) &
            "    ");
      end loop;
      New_Line;
   ...
   procedure print_debug_genome (s : solution_t2p_t2c) is
   begin
