...
/* function callback to specified the type of anomaly detected */
void* my_callback(int anomaly_number, pthread_monano_id_t id)
{
       printf("Type anomaly : %d Anomaly is detected on thread number %d\n", anomaly_number, id);
       pthread_monano_stop(&my_monano);
...
void run_monano()
{
  ...
  struct pthread_monano_attr_t tparam []={{99,{2,0},{4,0},{4,0}}}; 
