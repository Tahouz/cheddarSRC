...
void*  periodic_activation(void* arg)
{
  ...
  printf("Thread %x: lock for non preemptive \n", pthread_self());
  if(status<0)
  ...
  printf("Thread %x: unlock for non preemptive \n", pthread_self());
...
void run_rosace(void){
  ...
  struct pthread_monano_attr_t tparam []={{99,{2,0},{4,0},{20,0},{3,0}},
					  {98,{4,0},{9,0},{20,0},{0,0}}, {97,{10,0},{18,0},{20,0},{0,0}}}; 
  int status, i;
  int *ego;
  ...
  // Initialize the pthread_mutex for non preemptive
