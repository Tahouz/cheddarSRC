...
int elevator489_fun(void* args)
{
  ...
  static int instance=0;
  struct timespec starttime, executiontime, currenttime, begincriticaltime, endcriticaltime;
  int status, lock=0;
#ifdef CONFIGURE_MONANO_RTEMS48
 #ifdef CONFIGURE_MONANO_INCREASE_PROCESSOR_SPEED 
  executiontime.tv_sec=8; // reduce from 16 to 8
  executiontime.tv_nsec=0;
  begincriticaltime.tv_sec=1; // reduce from 2 to 1
  begincriticaltime.tv_nsec=0;
  endcriticaltime.tv_sec=6; // reduce from 12 to 6
  endcriticaltime.tv_nsec=0;
 #endif
 #ifdef CONFIGURE_MONANO_TASKS_EXECUTION_TIME 
  printf("Thread %x ELEVATOR is running \n", pthread_self());
  ...
  do
  {
	...
	if((ts_compare(currenttime,ts_add(starttime,begincriticaltime))==1)&&(lock==0)){ 
	  lock=1;
	...
	if((ts_compare(currenttime,ts_add(starttime,endcriticaltime))==1)&&(lock==1)){
	  lock=2;
	  printf("Thread %x: unlock critical resource \n", pthread_self());
...
#ifdef CONFIGURE_MONANO_RTEMS48
 #ifdef CONFIGURE_MONANO_INCREASE_PROCESSOR_SPEED 
  executiontime.tv_sec=3;
  executiontime.tv_nsec=0;
  begincriticaltime.tv_sec=1;
  begincriticaltime.tv_nsec=0;
  endcriticaltime.tv_sec=2;
  endcriticaltime.tv_nsec=0;
 #endif
 
 #ifdef CONFIGURE_MONANO_TASKS_EXECUTION_TIME
  printf("Thread %x ENGINE is running \n", pthread_self());
