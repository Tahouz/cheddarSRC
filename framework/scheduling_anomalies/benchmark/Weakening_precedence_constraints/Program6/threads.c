...
void run_rosace(void)
{
  ...
  struct pthread_monano_attr_t tparam []={{97,{2,0},{3,0},{3,0}},
      {99,{1,0},{4,0},{6,0}}, {93,{1,0},{4,0},{6,0}}}; 

  struct pthread_monano_precedency_t pred;
  int status, i;
  int *ego;
  ...
  status=sem_init(&id2_id1,0,0);
  if(status<0)
    returncode("sem_init ",status);
   
  /* Monano init and register */
  pthread_monano_init(&my_monano);
  pthread_monano_attr_init(&my_monano_attr);
  pthread_monano_precedency_attr_setsource(&pred, 2);
  pthread_monano_precedency_attr_setdestination(&pred, 1);

  pthread_monano_add_pthread_precedencies(&my_monano, pred);
