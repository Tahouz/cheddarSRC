...
int elevator489_fun(void* args)
{
  ...
  static int instance=0;
  struct timespec executiontime, resourcetime, currenttime, delaytime, starttime;
  int status, t=0, lock=0;

#ifdef CONFIGURE_MONANO_RTEMS48
 #ifdef CONFIGURE_MONANO_TASKS_EXECUTION_TIME
  printf("Thread %x is running \n",pthread_self());  
  executiontime.tv_sec=8;
  executiontime.tv_nsec=0;
  resourcetime.tv_sec=4;
  ...
  do
  {
   ...
   #ifdef CONFIGURE_MONANO_DELAY_TASK_EXECUTION
	if((ts_compare(currenttime, ts_add(starttime,delaytime))!=1)&&(t==0)){
	...
	if ((ts_compare (currenttime, ts_add(starttime,resourcetime))==1)&&(lock==0)){
	  lock=1;
	  status = pthread_mutex_lock(&shared_resources_engine_elevator);
	  printf("Thread %x: lock resource \n", pthread_self());
	  if(status<0)
	    returncode("pthread_mutex_lock ", status);  		
	}
	if (lock==1)
	  printf("Thread %x:is using the resource \n", pthread_self());
	
	if((ts_compare (currenttime, ts_add(starttime,executiontime))!=1)&&(lock==1)){
	  lock=2;
	  printf("Thread %x: end the using the resource \n", pthread_self());
	  printf("Thread %x: unlock resource \n", pthread_self());
	  status = pthread_mutex_unlock(&shared_resources_engine_elevator);
	  if(status<0)
...
int engine486_fun(void* args)
{
  ...
  static int instance=0;
  struct timespec starttime, executiontime,  currenttime, resourcetime;
  int status, lock=0;
  
#ifdef CONFIGURE_MONANO_RTEMS48
 #ifdef CONFIGURE_MONANO_TASKS_EXECUTION_TIME
   printf("Thread %x is running \n",pthread_self());
  executiontime.tv_sec=9;
  executiontime.tv_nsec=0;
  resourcetime.tv_sec=1;
  resourcetime.tv_nsec=0;
