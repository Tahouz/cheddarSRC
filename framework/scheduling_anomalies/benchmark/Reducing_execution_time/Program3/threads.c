...
/*mutex for manage the shared resources  
    between q_filter and vz_filter */
pthread_mutex_t shared_ressources_q_filter_vz_filter;
...
void run_rosace(void)
{
  ...
  struct pthread_monano_attr_t tparam []={{97,{6,0},{16,0},{16,0}},{99,{2,0},{6,0},{8,0}},
					{98,{6,0},{15,0},{16,0}}}; 
  int status, i;
  int *ego;
  ...
  status=pthread_mutex_init(&shared_ressources_q_filter_vz_filter, NULL);
  if(status<0)
