...
int az_filter_100458_fun(void* args)
{
  ...
  executiontime.tv_sec=5; //reduce from 6 to 5
  ...
  //do ... while to repeat until execution time of the task
  do 
  {
...
int Vz_filter_100452_fun(void* args)
{
...
#ifdef CONFIGURE_MONANO_RTEMS48
 #ifdef CONFIGURE_MONANO_SHARED_RESOURCES
  status = pthread_mutex_lock(&shared_ressources_q_filter_vz_filter);
  printf("Thread %x: LOCK RESOURCE  \n", pthread_self());
  if(status<0)
 ...
 #ifdef CONFIGURE_MONANO_SHARED_RESOURCES
  printf("Thread %x: UNLOCK RESOURCE  \n",  pthread_self());
  status = pthread_mutex_unlock(&shared_ressources_q_filter_vz_filter);
  if(status<0)
