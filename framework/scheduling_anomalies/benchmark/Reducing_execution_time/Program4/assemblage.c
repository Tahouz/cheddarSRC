...
int Va_control_50474_fun(void* args)
{
  ...
  static int instance=0;
  struct timespec starttime, executiontime, currenttime, waittime;
  int status, t=1;
 ...
 #ifdef CONFIGURE_MONANO_SUSPENDED_TASKS     
    if ((ts_compare(currenttime,ts_add(starttime, waittime))==1)&&(t==1)){
      printf("Thread %x is suspended \n", pthread_self());
  ...
  while (ts_compare (currenttime, ts_add(ts_add(starttime, executiontime),waittime))!=1) ;  
    instance++;
 #endif
#endif 
...
{
  static int instance=0;
  struct timespec starttime, executiontime, waittime, currenttime;
  int status, t=1;
...
int q_filter_100455_fun(void* args)
{
  ...
  executiontime.tv_sec=3; //reduce from 4 to 3
  executiontime.tv_nsec=0;  
 #endif
