...
//phtread mutex for manage the non premptive of each task
pthread_mutex_t non_preemptive;
...
/*Semaphore for manage the precedence constraints 
    between h_filter and altitude_hold */
//pthread_mutex_t pred_h_filter_altitude_hold;
//sem_t pred_h_filter_altitude_hold;
...
void*  periodic_activation(void* arg)
{
 ...
 #ifdef CONFIGURE_MONANO_NON_PREEMPTIVE_SCHEDULING
  status = pthread_mutex_lock(&non_preemptive);
  printf("Thread %x: Lock for no preemption  \n",  pthread_self());
  if(status<0)
...
void run_rosace(void)
{
  ...
  struct pthread_monano_attr_t tparam []={{99,{1,0},{3,0},{3,0}},{98,{2,0},{6,0},{6,0}},
					{97,{4,0},{12,0},{12,0}}}; 
  int status, i;
  int *ego;
  ...
  // Initialize the pthread_mutex non_preemptive
  status=pthread_mutex_init(&non_preemptive,NULL);
  if(status<0)
    returncode("pthread_mutex_init %d \n ",status);
