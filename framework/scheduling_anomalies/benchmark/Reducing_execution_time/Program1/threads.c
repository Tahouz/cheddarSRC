...
void run_rosace(void)
{
  ...
  struct pthread_monano_attr_t tparam []={{97,{5,0},{18,0},{18,0}},{98,{2,0},{6,0},{6,0}},
					{96,{1,0},{18,0},{18,0}},{99,{5,0},{18,0},{18,0}}}; 
  ...
  // Initialize the semaphore for precedence constraint pred_h_filter_altitude_hold
  status=sem_init(&pred_h_filter_altitude_hold,0,0);
  if(status<0)
