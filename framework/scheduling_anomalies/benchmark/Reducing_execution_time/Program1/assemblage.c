...
/**
 * departuretime: time of the task departure
 * starttime : time of the beginning of the task execution
 * currenttime: current time during the task execution
 * endtime : time of the end of the task execution
 * executiontime : the effective execution time of the task
 * */
...
int altitude_hold_50464_fun(void* args)
{
  ...
  static int instance=0;
  struct timespec starttime, executiontime, currenttime;
  int status;
#ifdef CONFIGURE_MONANO_RTEMS48 
 
 #ifdef CONFIGURE_MONANO_PRECEDENCE_CONSTRAINTS
  status = sem_wait(&pred_h_filter_altitude_hold);
  if(status<0)
  ...
  executiontime.tv_sec=5;
  executiontime.tv_nsec=0;	 
  do 
  {
  ...
  while (ts_compare (currenttime, ts_add(starttime, executiontime))!=1) ;   
 #endif
#endif
...
#ifdef CONFIGURE_MONANO_RTEMS48 
 #ifdef CONFIGURE_MONANO_REDUCE_EXECUTION_TIME
  executiontime.tv_sec=3; // reduce from 5 to 3
  executiontime.tv_nsec=0;
 #endif
...
int h_filter_100446_fun(void* args)
{
 ...
 #ifdef CONFIGURE_MONANO_PRECEDENCE_CONSTRAINTS
  status = sem_post(&pred_h_filter_altitude_hold);
  if(status<0)
    returncode("sem_post   ",status);
 #endif
#endif 
