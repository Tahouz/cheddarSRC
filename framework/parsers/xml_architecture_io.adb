...
begin
	if Get_Length (Atts) > 0 then
		if (To_String (To_Lower (Qname)) = "cfg_edge")
			 ...
			 OR (To_String (To_Lower (Qname)) = "scheduling_parameters") then
...
begin 	obj.type_of_dependency := Dependency_Type'first;
	...
	obj.tdma_communication_dependent_task := empty_string;
	obj.tdma_communication_orientation := Orientation_Dependency_Type'first;
	obj.tdma_communication_dependency_object := empty_string;
...
begin
	if Get_Length (Atts) > 0 then
		...
		if To_String (To_Lower (Qname)) = "tdma_communication_dependent_task" then
			for J in 0 .. Get_Length (Atts) - 1 loop
				if To_String (To_Lower (Get_Qname (Atts, J))) = "ref" then
					obj.tdma_communication_dependent_task := To_Unbounded_String (Get_Value (Atts, J));
				end if;
			end loop;
		end if;

		if To_String (To_Lower (Qname)) = "tdma_communication_dependency_object" then
			for J in 0 .. Get_Length (Atts) - 1 loop
				if To_String (To_Lower (Get_Qname (Atts, J))) = "ref" then
					obj.tdma_communication_dependency_object := To_Unbounded_String (Get_Value (Atts, J));
				end if;
			end loop;
		end if;
...
begin
	...
	if To_String (To_Lower (Qname)) = "tdma_communication_orientation" then
		To_Orientation_Dependency_Type (handler.Parameter_List (1), obj.tdma_communication_orientation, Handler.Ok);
		if not Handler.Ok then
			Put_Line ("Warning : Error on data type From " & To_String (Handler.Locator));
		end if;
	end if;
