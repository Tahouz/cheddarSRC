# Task Placement Algorithm: Rate-Monotonic Next-Fit

## Description

In assigning tasks to processors, Algorithm (RMNF) only checks the current processor to see
whether a task together with those tasks that have already been assigned to that processor can be
feasibly scheduled or not using ***(IP Condition)*** . If not, the task has to be scheduled on an idle processor .
...
$U_{\text{cpu2}} = 0.38 \le n \left(2^{\frac{1}{n}} - 1\right) \simeq 0.82 => True$

$C_m / T_m \le 2(1 + u_{\text{cpu2}} / (m-1))^{-(m-1)} -1 => 0.20 \le 0.6 => True$

<sub>we add T5 to **cpu2** :</sub>


<p style="text-align:center;"> <strong>cpu2 : {T4 , T5}</strong> </p>
...
```
/cheddar_cli \ 
--request=scheduling_feasibility_next_fit \
--file=next_fit_test_case.xml 
```

![Cheddar-GUI](images/next-fit.png)
[Download XML](./xmls/next_fit_test_case.xml)
